...
func main() {
	...
	signedSecEvent, err := generateSignedSessionRevokedSecEvent(keyPair.PrivateKey)
	if err != nil {
	...
	secEventParser := parser.NewParser(
		parser.WithPublicKey(keyPair.PublicKey, "key-1"),
	...
	if caepEvent, ok := event.(*caep.SessionRevokedEvent); ok {
...
func generateSignedSessionRevokedSecEvent(privateKey *ecdsa.PrivateKey) (string, error) {
